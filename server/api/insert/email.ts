export default defineEventHandler(async (event) => {
	try {
		...
		const { email, auditorios } = await readBody(event) as { email: string; auditorios: string[] }
		if (['Administrador'].includes(user.level)) {
			await criarEmail(email, auditorios)
			new Log({
				usuario: user.idcbpf,
				acao: `cadastrou o email '${email}' para receber os emails dos auditórios '${auditorios.join(', ')}'`,
			}).save()
			return 'Ok'
		}
		throw {
