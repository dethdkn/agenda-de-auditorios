export default defineEventHandler(async (event) => {
	try {
		...
		if (['Administrador'].includes(user.level)) {
			...
			let planta: string | undefined
			if (plantaFile)
				planta = await salvarImagem(plantaFile)
			let itensArr: string[] = []
			if (itens) {
				itensArr = itens
					.split(',')
			...
			if (nome && capacidade && coordenacao && url) {
				await criarAuditorio(nome, capacidade, coordenacao, url, itensArr, descricao, planta)
				new Log({
					usuario: user.idcbpf,
					acao: `Criou o auditorio ${nome}`,
				}).save()
				return 'Ok'
			}
			throw 'Erro ao criar auditório'
		}
		throw {
			statusCode: 403,
