export default defineEventHandler(async (event) => {
	try {
		...
		const filtro: any = {}
		if (auditorio && auditorio !== 'Selecione') filtro.audNome = auditorio
		if (coordenacao && coordenacao !== 'Selecione') filtro.audCoord = coordenacao
		if (status && status !== 'Selecione') filtro.status = status
		if (data && data !== '') filtro['datas.data'] = new Date(data)
		if (['Administrador', 'Gerente', 'Secretária', 'Técnico', 'Externo'].includes(user.level)) {
			if (['Secretária', 'Externo'].includes(user.level)) filtro.solicitadorPor = user.idcbpf
