export default defineEventHandler(async (event) => {
	try {
		...
		const {id, email} = JSON.parse(await readBody(event)) as {id: string; email: string}
		if (['Administrador'].includes(user.level)) {
			await excluirExterno(id)
			new Log({usuario: user.idcbpf, acao: `Excluiu o usuário externo ${email}`}).save()
			return ''
		}
		throw {
			statusCode: 403,
