export default defineEventHandler(async (event) => {
	...
	try {
		if (coord)
			return await gerarICS(await fetchReservasAprovadasPorCoord(coord))
		if (aud)
			return await gerarICS(await fetchReservasAprovadasPorAuditorio(aud))
		if (uid)
			return await gerarICS(await fetchReservasAprovadasPorSolicitante(uid))
