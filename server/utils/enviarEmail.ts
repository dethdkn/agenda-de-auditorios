const isDev = useRuntimeConfig().public.DEV_MODE === 'false'

export default (
	status: 'Aprovado' | 'Recusado' | 'Aguardando' | 'Cancelado',
	solicitante: string | string[],
	coordSolicitante: string[],
	audNome: string,
	audCoord: string,
	nomeEvento: string,
	tipoEvento: string,
	...
	return new Promise(async (resolve, reject) => {
		...
		else for (const solic of solicitante) dadosMail.to.push(isEmail(solic) ? solic : await getUserMail(solic))

		const datasReserva: string[] = []
		for (const data of datas) datasReserva.push(data.data.toLocaleDateString('pt-br'))

		if (status === 'Aprovado') {
			dadosMail.subject = `${dadosMail.subject} Reserva Aprovada - ${audNome} | ${datasReserva.join(' | ')}`

			if (isDev) {
				const mails = await fetchEmails()
				for (const mail of mails) {
					if (mail.auditorios.includes(audNome))
						dadosMail.to.push(mail.email)
				}
			}
			...
			for (const usuario of usuarios) {
				...
				if (usuario.nivel === 'Técnico' && isDev)
					dadosMail.to.push(`${usuario.idcbpf}@cbpf.br`)
				if (usuario.nivel === 'Gerente' && usuario.coordenacao.includes(audCoord) && isDev)
					dadosMail.to.push(`${usuario.idcbpf}@cbpf.br`)
				for (const coord of usuario.coordenacao) {
					if (coordSolicitante.includes(coord) && ['Gerente', 'Secretária'].includes(usuario.nivel) && isDev)
						dadosMail.to.push(`${usuario.idcbpf}@cbpf.br`)
				}
			}

			let datasTexto = `<ul>`
			for (const data of datas) datasTexto = `${datasTexto}<li>${data.data.toLocaleDateString('pt-br')} das ${data.inicio} às ${data.fim}</li>`
			datasTexto = `${datasTexto}</ul>`

			let recursosTexto = `<ul>`
			for (const recurso of recursosSolicitados) recursosTexto = `${recursosTexto}<li>${recurso}</li>`
			...
			dadosMail.subject = `${dadosMail.subject} Reserva Recusada - ${audNome} | ${datasReserva.join(' | ')}`
			...
			dadosMail.subject = `${dadosMail.subject}Solicitação de Reserva - ${audNome} | ${datasReserva.join(' | ')}`
			...
			dadosMail.subject = `${dadosMail.subject}Reserva Cancelada - ${audNome} | ${datasReserva.join(' | ')}`
		...
		else { return reject('status do auditório inválido') }

		dadosMail.to = [...new Set(dadosMail.to)].sort()
