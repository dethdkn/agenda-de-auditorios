...
const opcoesFiltro = computed(() => {
	...
	if (userStore().idcbpf)
		filtros.push('Solicitados por mim')
...
function limparFiltros() {
	...
	if (filtro.value === 'Solicitados por mim')
		usuarioSelecionado.value = userStore().idcbpf
...
function abrirDetalhes({ year, month, day }: { year: number; month: number; day: number }) {
	...
	if (data.value) {
		reservasDetalhes.value = (data.value as unknown as Reserva[]).filter((entry) => {
			for (const dbData of entry.datas) {
				if (new Date(dbData.data).getTime() === dataDetalhes.value.getTime())
					return true
			}
...
<template>
	...
	>
		...
		<ClientOnly>
			<template #fallback>
				<USkeleton class="w-full h-96" :ui="{ background: 'bg-gray-300 dark:bg-gray-600' }" />
			</template>
			<Calendar
				expanded
				locale="pt-BR"
			...
			>
				<template #day-popover="{ attributes }">
					<div v-for="(atributo, i) in attributes" :key="i" class="flex items-center justify-start mx-2 my-1">
						<span class="hidden">
				...
				<UPopover mode="hover" :ui="{ width: 'max-w-md' }">
					...
					<template #panel>
						<div class="p-6">
							...
							<ul class="list-disc">
								<li class="pb-3">
									Clique no botão abaixo para copiar a URL do calendário
								</li>
								<li class="pb-3">
	...
	<UModal v-model="modalDetalhes" :ui="{ width: '!w-4/5 sm:max-w-full' }">
		<UCard
			:ui="{
		...
		>
			...
			<UAccordion v-if="reservasDetalhesItem.length > 0" :items="reservasDetalhesItem as AccordionItem[]">
				<template #default="{ item, open }">
					...
					>
						<template #leading>
							<div
								class="flex items-center justify-center w-6 h-6 -my-1 rounded-full bg-green-500"
							>
					...
					<UContainer>
						<div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-10 text-center place-items-center">
							...
							<div>
								...
								<ul class="list-disc">
									<li v-for="(dataHr, i) in item.datashoras" :key="i">
										{{ dataHr }}
									</li>
								</ul>
							</div>
