...
const idRecusar = ref('')
const abrirRecusar = (id?: string) => {
	if (id) idRecusar.value = id
	modalRecusar.value = true
}
watch(modalRecusar, (nv) => {
	if (!nv) idRecusar.value = ''
})
const recusar = () => {
	loadingRecusar.value = true
	fetch('/api/update/recusar', {
		method: 'POST',
		body: JSON.stringify({
			id: idRecusar.value
		})
...
const idAprovar = ref('')
const abrirAprovar = (id?: string) => {
	if (id) idAprovar.value = id
	modalAprovar.value = true
}
watch(modalAprovar, (nv) => {
	if (!nv) idAprovar.value = ''
})
const aprovar = () => {
	loadingAprovar.value = true
	fetch('/api/update/aprovar', {
		method: 'POST',
		body: JSON.stringify({
			id: idAprovar.value
		})
...
const abrirCancelar = (id?: string) => {
	if (id) idCancelar.value = id
	modalCancelar.value = true
}
watch(modalCancelar, (nv) => {
	if (!nv) idCancelar.value = ''
})
const cancelar = () => {
	loadingCancelar.value = true
	fetch('/api/update/cancelar', {
		method: 'POST',
		body: JSON.stringify({
			id: idCancelar.value
		})
...
<template>
			...
			<div class="flex items-center justify-start">
				...
				<div>
					...
					<USelectMenu
						...
						searchable-placeholder="Pesquisar..."
					/>
					<VDatePicker v-if="filtro === 'Data'" class="ms-4" v-model="dataSelecionada" />
				</div>
			</div>
		</template>
		<UAccordion :items="reservaListaPag">
			...
			<template #item="{item}">
				<UContainer>
					<div class="grid grid-cols-3 gap-4 mb-10 text-center place-items-center">
						...
						<div>
							<p class="text-xl font-extrabold text-black dark:text-white">E-mail do Responsável</p>
						...
						<div>
							<p class="text-xl font-extrabold text-black dark:text-white">Status da Reserva</p>
							<p>{{ item.status }}</p>
						</div>
						<div>
						...
						<div>
							<p class="text-xl font-extrabold text-black dark:text-white">Aceito/Recusado Por</p>
							<p>{{ item.aceitoPor }}</p>
						</div>
						<div>
							<p class="text-xl font-extrabold text-black dark:text-white">Recursos do Auditório</p>
						...
						<div>
							<p class="text-xl font-extrabold text-black dark:text-white">Observação</p>
							<p>{{ item.observacao }}</p>
						</div>
					</div>
					<div
	...
	<UModal v-model="modalRecusar" :ui="{width: '!w-2/5'}">
		...
		>
			<template #header>
				<h3 class="text-center text-white select-none">Recusar Solicitação de Reserva</h3>
			</template>
			<span
	...
	<UModal v-model="modalAprovar" :ui="{width: '!w-2/5'}">
		...
		>
			<template #header>
				<h3 class="text-center text-white select-none">Aprovar Solicitação de Reserva</h3>
			</template>
			<span
				class="block mb-2 text-sm font-medium text-center text-gray-900 dark:text-white"
				for="excluir-form"
	...
	<UModal v-model="modalCancelar" :ui="{width: '!w-2/5'}">
		...
		>
			<template #header>
				<h3 class="text-center text-white select-none">Cancelar Reserva</h3>
			</template>
			<span
				class="block mb-2 text-sm font-medium text-center text-gray-900 dark:text-white"
				for="excluir-form"
