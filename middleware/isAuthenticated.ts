export default defineNuxtRouteMiddleware(async (to) => {
	if (await useCheckAuth()) {
		if (['Administrador'].includes(userStore().level)) {
			if (['/auditorios', '/usuarios', '/emails', '/solicitar-reserva', '/reservas'].includes(to.fullPath))
				return
			return navigateTo('/')
		}
		else if (['Gerente'].includes(userStore().level)) {
			if (['/solicitar-reserva', '/reservas'].includes(to.fullPath))
				return
			return navigateTo('/')
		}
			...
			if (['/reservas'].includes(to.fullPath))
				return
			return navigateTo('/')
		}
		else {
